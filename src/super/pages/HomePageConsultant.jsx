...
import { getBookingConsultant } from "api/expert";
...
const fakeData = {
  ...
  "2022-04-19": [{ time: "13:00 - 14:00", student: "Nguyen Thi E" }],
  "2022-04-20": [
...
export default function WeekPicker({ userid }) {
  ...
  useEffect(() => {
    const fetchBookings = async (day) => {
      try {
        ...
        const response = await getBookingConsultant({
          userid,
          day: formattedDay,
        });
        setBookingData((prevData) => ({
          ...prevData,
          [formattedDay]: response.bookings,
        }));
      } catch (error) {
  ...
  return (
    <Box>
      <Typography
        variant="h6"
        sx={{ pb: 2, textAlign: "center", backgroundColor: "aliceblue" }}
      >
        Chọn lịch Tuần
      </Typography>
            ...
            <React.Fragment key={day.format("YYYY-MM-DD")}>
              <Box
                sx={{
                  ...
                  borderRadius: "5px",
                  cursor: "pointer",
                  backgroundColor: activeDays[day.format("YYYY-MM-DD")]
                    ? "#e0f7fa"
                    : "white",
                }}
              ...
              {activeDays[day.format("YYYY-MM-DD")] && (
                <Box
                  sx={{ mt: 1, ml: 3, pl: 2, borderLeft: "2px solid #22c55e" }}
                >
                  <Text variant="body2" sx={{ fontWeight: "bold", mb: 1 }}>
                    Timeslots
                  </Text>
                  {(fakeData[day.format("YYYY-MM-DD")] || []).map(
                    (slot, index) => (
                      <Box
                        key={index}
                        sx={{
